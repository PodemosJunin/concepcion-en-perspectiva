...
function svgCompare(ind, opts={}){
  ...
  s+=`<text class="chart-label" x="${W-210}" y="18">Concepción</text><line class="chart-line" x1="${W-220}" x2="${W-190}" y1="14" y2="14"/><text class="chart-text" x="${W-92}" y="18">Perú</text><line class="chart-line secondary" x1="${W-132}" x2="${W-102}" y1="14" y2="14"/>`;
  s+='</svg>';return s;
}
...
function chartMove(e,hit){
  ...
  tip.innerHTML=`<div class="tooltip-year">${yr}</div>${gestion}<div class="tooltip-row"><span><i></i>Concepción</span><strong>${fmt(ind,val,1)}</strong></div>${peruRow}`;
...
function nav(){
 ...
 h.innerHTML=`<header class="site-header"><div class="container nav-wrap"><a class="brand" href="index.html"><span class="brand-mark brand-party"><img src="assets/logo-partido.png" alt="Logo" aria-hidden="true"></span><span class="brand-text">Jaime Córdoba Vila a la<br>Municipalidad Provincial de Concepción</span></a><button class="menu-toggle" aria-label="Abrir menú" aria-expanded="false">☰</button><nav class="nav-links" aria-label="Navegación principal"><a href="index.html" class="${page==='inicio'?'active':''}">Inicio</a><div class="nav-dropdown ${dimActive?'active':''}"><a class="nav-dropbtn" href="index.html#dimensiones" aria-haspopup="true" aria-expanded="false">Dimensiones <span aria-hidden="true">▾</span></a><div class="nav-menu" role="menu"><a role="menuitem" href="social.html">Dimensión social</a><a role="menuitem" href="economia.html">Dimensión económica</a><a role="menuitem" href="ambiental.html">Dimensión ambiental</a><a role="menuitem" href="institucional.html">Dimensión institucional</a></div></div><a href="comparar.html" class="${page==='comparar'?'active':''}">Comparar periodos</a><a href="junin.html" class="${page==='junin'?'active':''}">Concepción frente a Junín</a><a href="datos.html" class="${page==='datos'?'active':''}">Indicadores</a><a href="metodologia.html" class="${page==='metodo'?'active':''}">Metodología</a></nav></div></header>`;
...
function footer(){const el=document.querySelector('[data-footer]');if(!el)return;el.innerHTML=`<footer class="footer"><div class="container footer-grid"><div><strong>Concepción en perspectiva</strong><p>Observatorio provincial de Concepción con indicadores públicos, comparación de periodos y finanzas municipales.</p></div><div><strong>Explorar</strong><p><a href="index.html#dimensiones">Dimensiones</a><br><a href="comparar.html">Comparar periodos</a><br><a href="junin.html">Concepción frente a Junín</a><br><a href="datos.html">Indicadores</a></p></div><div><strong>Transparencia</strong><p><a href="metodologia.html">Fuentes y metodología</a></p></div></div></footer>`}

function mascot(){
  const key='concepcion_mascota_oculta_v3';
  const posKey='concepcion_mascota_pos_v3';
  try{if(localStorage.getItem(key)==='1')return}catch(_){ }
  if(document.querySelector('.mascot-widget'))return;
  const facts=[
    'Jaime Córdoba Vila es la mejor opción para Concepción.',
    'Concepción necesita una gestión cercana, ordenada y enfocada en resultados.',
    'Conoce los datos de la provincia y revisa sus principales brechas.',
    'Con Jaime Córdoba Vila, Concepción puede dar el siguiente paso.',
    'Este 4 de octubre, marca la P de Podemos Perú.'
  ];
  const w=document.createElement('aside');
  w.className='mascot-widget';
  w.setAttribute('aria-label','Mensaje de campaña de Jaime Córdoba Vila');
...
function chartClone(id){
  ...
  st.textContent=`
    ...
    .district-shape{stroke:#7393A3;stroke-width:.65;stroke-linecap:round;stroke-linejoin:round;vector-effect:non-scaling-stroke}.district-shape.is-concepcion{stroke:#003C5C;stroke-width:2.4}
  `;
